...
const DATA_DIR = process.env.RAILWAY_VOLUME_MOUNT_PATH || path.join(__dirname, 'data');  // volumen persistente en Railway; local usa ./data
...
const server = http.createServer(async (req,res)=>{
  ...
  if(u.split('?')[0]==='/api/relay'){
    if(req.method==='POST'){ const b=await readBody(req); apexUrl=(b.url||'').toString().trim(); apexUserOrigin=(b.origin||'').toString().trim(); stateLines={}; lastClock=null; allFrames.length=0; try{if(upstream)upstream.terminate();}catch(_){}; upstream=null; upRetry=0; injectMode=isPlayerUrl(apexUrl); if(apexUrl && !injectMode) connectApex(); if(injectMode) console.log('[relay] modo REPRODUCTOR: uso el feed inyectado en /inject'); if(b.session){ const s=sessions.find(x=>x.id===b.session); if(s){ s.url=apexUrl; s.origin=apexUserOrigin; saveSessions(sessions); } } return sendJson(res,{ok:true,url:apexUrl}); }
    return sendJson(res,{url:apexUrl, mode:(injectMode?'player':'apex'), live:(injectMode?(injectWss&&injectWss.clients.size>0):!!(upstream&&upstream.readyState===1)), peers:feedWss?feedWss.clients.size:0});
  }
  // estático
  safeStatic(res,u);
});
...
let injectMode = false;                                                       // true = la fuente es el REPRODUCTOR (inyecta por /inject), no Apex real
// URL del reproductor: la que muestra su panel. El servidor NO marca hacia fuera, usa el feed inyectado.
function isPlayerUrl(u){ return /(^|\/)inject\/?(\?|$)/i.test(String(u||'')) || /^mrp-player$/i.test(String(u||'').trim()); }
